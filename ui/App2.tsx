// https://gist.github.com/keshihoriuchi/40ff3217a7a63d25788ce5cb8230ba3b
import { useEffect, MutableRefObject, useRef, createRef, useState } from "react";
import "./App.css";
...
const VideoRoom: React.FC = () => {
  ...
  const refPc1: MutableRefObject<RTCPeerConnection> = useRef(new RTCPeerConnection());
  const refPc2: MutableRefObject<RTCPeerConnection> = useRef(new RTCPeerConnection());
    ...
    const func = async () => {
      ...
      pc2.addEventListener("track", (e) => {
        ...
        if (refPeerTrackEvents.current.length === 2) {
          refPeerVideoElemRefs.current["peer"] = createRef();
          setPeer(true);
        }
      });
  ...
  return (
    <div>
      ...
      {peer ? <video ref={refPeerVideoElemRefs.current["peer"]} autoPlay /> : <div />}
    </div>
  );
};
